...
class Response(_BaseRequestResponse, JSONMixin):
    ...
    def __init__(
        ...
        try:
            ...
            raise ValueError(
                "Quart  does not support non-integer status values") from error
    ...
    async def make_conditional(
        ...
        if request_range is None or len(request_range.ranges) == 0:
            return
    ...
    def access_control_allow_credentials(self, value: bool) -> None:
        if value is True:
            ...
            self.headers.pop("Access-Control-Allow-Credentials",
                             None)  # type: ignore
    ...
    def content_security_policy(self, value: ContentSecurityPolicy) -> None:
        self._set_or_pop_header(
            "Content-Security-Policy", dump_csp_header(value))
